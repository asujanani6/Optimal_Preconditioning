function run_Rand_Stan_three_comp(dimvec,seedvec,paramsUs,optionsUs,paramsStan,paramsUsSimp,filename)
...
fmt3 = '%6.1e & %6.1e & %6.1e & %5.3f & %5.3f & %5.3f & %4.1f & %4.1f';
fmt4 = ' \\cr\\hline\n';
fmt5 = '\\cr\\cline{1-3}\\cline{3-11}\n';
...
fprintf(fid, '%s', [...
    ...
    '\multicolumn{3}{|c|}{\% Reduction in $\kappa$} & ', ...
    '\multicolumn{3}{|c|}{CPU Time (seconds)} &', ...
    '\multicolumn{2}{|c|}{CPU Ratios}']);
fprintf(fid, fmt5);
fprintf(fid, hfmt1, '$n$', 'density', '$\kappa(A)$');
fprintf(fid, hfmt3,'Qu et al.', 'Alg 2.2', 'Alg. 3.1', ...
    'Qu et al.', 'Alg 2.2', 'Alg 3.1', ...
    'Qu/Alg 2.1', 'Qu/Alg3.1');
fprintf(fid, fmt4);

for k=1:numofProblems
    fprintf(fid, fmt1, dimA(k), densityA(k), kappaA(k));
    fprintf(fid, fmt3, kappa_Reduc_Stan(k), kappa_Reduc_UsSimp(k), kappa_Reduc_US(k), ...
        Stan_kappa_time(k), UsSimp_kappa_time(k), US_kappa_time(k), ...
