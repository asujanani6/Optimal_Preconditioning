function run_for_table_5(dimvec,seedvec,paramsUs,optionsUs, paramsStan, filename)
...
%%%Save Total time: Us vs Stanford
US_total_time=zeros(numofProblems,1);
St_total_time=zeros(numofProblems,1);
...
for i=1:numofProblems
    ...
    US_total_time(i)=US_PCG_CPU(i)+US_kappa_time(i);
    St_total_time(i)=Stan_PCG_CPU(i)+Stan_kappa_time(i);
...
fmt3 = '%6.1e & %6.1e & %5.0f & %5.0f & %4.2f & %4.2f & %4.2f & %4.2f';
fmt4 = ' \\cr\\hline\n';
...
fprintf(fid, hfmt3,'\cite{doi:10.1287/opre.2022.0592}', '\Cref{alg:GDeplustVv}', ...
    '\cite{doi:10.1287/opre.2022.0592}', '\Cref{alg:GDeplustVv}', ...
    '\cite{doi:10.1287/opre.2022.0592}', '\Cref{alg:GDeplustVv}', '\cite{doi:10.1287/opre.2022.0592}', '\Cref{alg:GDeplustVv}');
fprintf(fid, fmt4);
...
for k=1:numofProblems
    ...
    fprintf(fid, fmt3, kappa_Reduc_Stan(k), kappa_Reduc_US(k), ...
        ...
        Stan_PCG_CPU(k), US_PCG_CPU(k), St_total_time(k), US_total_time(k));
    fprintf(fid, fmt4);
end
